...
class ProfileController extends BaseController
{
    ...
    private function processProfileSave(Request $request, User $userModel, array $currentUser, int $userId): void
    {
        ...
        if ($bankAccount !== '' && !preg_match('/^\d{1,20}$/', $bankAccount)) {
            $errors[] = 'Bank account number must contain only digits (up to 20).';
        }

        if ($nic !== '' && !preg_match('/^[0-9Vv]{5,15}$/', $nic)) {
            $errors[] = 'NIC should be between 5 and 15 characters (numbers and V).';
        }
