  ...
  <div class="p-info-card">
    <div class="pc-card">
      ...
      <div class="profile-picture"
        ...
        <div class="form-group">
          ...
          <a href="<?= htmlspecialchars($metadata['document']) ?>" target="_blank">
            📄 View PDF
          </a>
        </div>
      ...
      <div class="form-group"><label>Age</label>
        <input type="text" value="<?= htmlspecialchars($metadata['age'] ?? '') ?>" disabled>
      </div>
    </div>
    ...
    <div class="pc-card">
      ...
      <div class="form-group"><label>Landline</label>
        <input name="country_code" value="<?= htmlspecialchars($metadata['country_code'] ?? '') ?>" required>

        <input type="tel" pattern="[0-9]{9}" value="<?= htmlspecialchars($metadata['land_phone'] ?? '') ?>" disabled>
      </div>
      <div class="form-group"><label>Address</label>
        <textarea disabled><?= htmlspecialchars($company['address'] ?? '') ?></textarea>
      </div>
      <div class="form-group"><label>Registered date</label>
        <input type="text" value="<?= !empty($company['registered_date'])
          ? date('Y-m-d H:i', strtotime($company['registered_date']))
          : 'N/A' ?>" disabled>
      </div>
    </div>
...
<div id="editModal" class="form-modal">
  <div class="form-modal-content">
    <a href="#" class="close"
      style="position: absolute; top: 30px; right: 20px; font-size: 28px; text-decoration: none;">&times;</a>
    ...
    <form id="editProfileForm" method="POST" enctype="multipart/form-data" action="/api/profile/update">
      ...
      <div class="form-group"><label>Registration Number<span class="required-star">*</span></label>
        <input type="text" name="reg_number" pattern="[A-Za-z][0-9]+[A-Za-z]"
          value="<?= htmlspecialchars($metadata['reg_number'] ?? '') ?>" required minlength="5" maxlength="10">
        <span class="field-error"></span>
      </div>
      ...
      <div class="form-group"><label>Age</label>
        <input type="text" name="age" pattern="[0-9]{3}|[0-9]{2}[a-z]"
          value="<?= htmlspecialchars($metadata['age'] ?? '') ?>">
        <span class="field-error"></span>
      </div>
      ...
      <div class="form-group"><label>Phone Number<span class="required-star">*</span></label>
        <input type="tel" pattern="07[0-9]{8}" maxlength="10" name="phone" placeholder="07XXXXXXXX"
          value="<?= htmlspecialchars($company['phone'] ?? '') ?>" required>
        <span class="field-error"></span>
      </div>

      <div class="form-group"><label>Landline</label>
        <select name="country_code" value="<?= htmlspecialchars($metadata['country_code'] ?? '') ?>" required>
          <option value="+94" <?= (isset($metadata['country_code']) && $metadata['country_code'] === '+94') ? 'selected' : '' ?>>+94</option>
          <option value="+91" <?= (isset($metadata['country_code']) && $metadata['country_code'] === '+91') ? 'selected' : '' ?>>+91</option>
        </select>
      ...
      <div class="form-group"><label>Registered date</label>
        <input type="datetime-local" name="registered_at" max="<?= date('Y-m-d\TH:i') ?>" value="<?= isset($company['registered_date'])
            ? date('Y-m-d\TH:i', strtotime($company['registered_date']))
            : '' ?>" required>
      </div>
...
/*build payload in profileController.php for reg date

'registered_at' => $request->input('registered_at')
? date('Y-m-d H:i:s', strtotime($request->input('registered_at')))
: null,
...
if ($registeredAt) {
$input = new DateTime($registeredAt);
$now = new DateTime();

if ($input > $now) {
$errors['registered_at'] = "Date cannot be in the future.";
}
}
*/
...
$file = $request->file('document');
        ...
        if ($documentPath) {
            $payload['metadata']['document'] = $documentPath;
        }