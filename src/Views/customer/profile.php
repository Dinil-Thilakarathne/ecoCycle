...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (isset($_POST['updatePassword'])) {
    ...
    if (!isset($customer['password'])) $customer['password'] = 'password123';
...
<style>
  ...
  /* Modal base */
  .form-modal { display: none; position: fixed; inset: 0; background: rgba(2,6,23,0.45); justify-content: center; align-items: center; z-index: 1000; padding: 1rem; }
  .form-modal.show { display: flex; }
  .form-modal-content { background: var(--card-bg); border-radius: 12px; max-width: 720px; width: 100%; padding: 1.25rem; box-shadow: 0 12px 40px rgba(2,6,23,0.12); position: relative; max-height: 90vh; overflow: auto; }
  .form-modal .close { position: absolute; right: 0.75rem; top: 0.5rem; font-size: 1.5rem; text-decoration: none; color: #333; }

  /* Cards and layout tweaks */
  .pc-card { background: var(--card-bg); border-radius: 12px; padding: 1.25rem; box-shadow: 0 6px 24px rgba(15,23,42,0.04); }
  .p-info-card { gap: 1.5rem; }

  /* Avatar */
  .avatar { width:120px; height:120px; object-fit:cover; border-radius:50%; border:4px solid rgba(34,197,94,0.08); box-shadow: 0 6px 16px rgba(34,197,94,0.06); }
  ...
  input[type="text"], input[type="email"], input[type="password"], textarea { width:100%; padding:0.55rem 0.75rem; border-radius:8px; border:1px solid #e6e9ee; background:#fff; font-size:0.95rem; color:#111827; box-sizing:border-box; }
  textarea { min-height:90px; resize:vertical; }
  input:focus, textarea:focus { outline: none; border-color: rgba(22,163,74,0.8); box-shadow: 0 4px 14px rgba(22,163,74,0.06); }

  /* File input */
  .input-file { width:100%; padding:0.45rem; border-radius:8px; border:1px dashed #d1d5db; background:#fbfdff; }

  /* Buttons */
  .btn { display:inline-block; font-weight:600; border-radius:8px; padding:0.55rem 0.9rem; cursor:pointer; text-align:center; border: none; transition:all 0.12s ease; }
  .btn-primary, .p-submit { background:var(--accent); color:#fff; box-shadow: 0 6px 18px rgba(16,185,129,0.08); }
  .btn-primary:hover, .p-submit:hover { background:var(--accent-600); transform: translateY(-1px); }
  .btn-outline { background:transparent; color:var(--accent); border:1px solid rgba(22,163,74,0.12); }
  .p-btn { background:transparent; border:1px solid #e6e9ee; color:#111827; padding:0.45rem 0.9rem; border-radius:8px; }
  .p-btn:hover { box-shadow: 0 8px 20px rgba(2,6,23,0.06); }
  .p-btn-delete { background:transparent; color:var(--danger); border:1px solid rgba(239,68,68,0.08); padding:0.5rem 0.9rem; border-radius:8px; }

  /* Bank grid */
  .bank-details-grid input { width:100%; }

  /* Utility */
  .page-header__title { color:#0f172a; }

  /* Toast */
  .toast { position: fixed; right: 20px; bottom: 24px; background: #052e19; color: #e6ffed; padding: 0.6rem 1rem; border-radius: 10px; box-shadow: 0 10px 30px rgba(2,6,23,0.15); font-weight:600; z-index:1200; }

  /* Form error messages */
  .form-error { color: #7f1d1d; background: #ffefef; padding: 0.5rem 0.75rem; border-radius: 6px; margin-bottom: 0.75rem; font-weight:600; border: 1px solid #fecaca; }

  @media (max-width: 480px) { .form-modal-content { padding: 0.75rem; max-width: 420px; } }
</style>
...
<main class="content">
  ...
  <div class="pc-card" style="width:100%;margin:0;box-shadow:0 2px 12px rgba(167,228,26,0.10);padding:1.5rem 1.2rem;box-sizing:border-box;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <button class="btn btn-outline" style="padding: 0.3rem 1.2rem; font-size: 0.95rem;" onclick="openBankModal()">Edit</button>
        </div>
